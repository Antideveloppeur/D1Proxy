package me.jordanamr.d1proxy.network;

import me.jordanamr.d1proxy.Proxy;
import me.jordanamr.d1proxy.handlers.PacketDestination;
import me.jordanamr.d1proxy.handlers.PacketHandler;
...
public class ProxyClient {
    ...
    public void sendMessage(String message) {
        if (state == ProxyClientState.INGAME && client.getChannel().isOpen()) Packet.builder().putBytes(("cs<font color='#2C49D7'>" + message + "</font>").getBytes(StandardCharsets.UTF_8)).putByte(0).queueAndFlush(client);
    }
    ...
    private void sendPacket(String packet, Client destination) {
        try {
            ...
            for (int bound = 0; bound < data.length; bound += 1024) {
                ...
                Packet.builder().putBytes(Arrays.copyOfRange(data, bound, end)).queueAndFlush(destination);
            }
        } catch (Exception ex) {
